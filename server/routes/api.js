...
router.get('/dep', function (req, res) {
    ...
    oracledb.getConnection(connAttrs, function (err, connection) {
        ...
        connection.execute("SELECT EMPLOYEE_ID, FIRST_NAME, EMAIL FROM EMPLOYEES WHERE EMPLOYEE_ID >2", {}, {
            outFormat: oracledb.OBJECT // Return the result as Object
        }, function (err, result) {
            if (err) {
                res.set('Content-Type', 'application/json');
        ...
        connection.execute("SELECT * FROM EMPLOYEES WHERE employee_id = :employee_id", [req.params.EMPLOYEE_ID], {
            ...
            if (err || result.rows.length < 1) {
                ...
                res.status(status).send(JSON.stringify({
                    status: status,
                    message: err ? "Error getting the employee" : "employee doesn't exist",
